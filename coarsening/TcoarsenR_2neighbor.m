function [coordinates,elements,irregular,varargout] = ...
    TcoarsenR_2neighbor(N0,coordinates,elements,irregular,varargin)
%TcoarsenR_2neighbor: local coarsening of triangular mesh emerged from  
%          TrefineRG and subsequent closure removement "coarse_greenelements"
%          where marked elements are coarsened by inverting the red refinement
%          and sticking to the 2-Neighbor-Rule
%
%Usage:
%
% [coordinates,elements3,dirichlet,neumann] ...
%    = TcoarsenR_2neighbor(N0,coordinates,elements3,dirichlet,neumann,marked)
% or
%
% [coordinates,elements3,irregular] ...
%    = TcoarsenR_2neighbor(N0,coordinates,elements3,irregular,marked)
%
%Comments:
%
%    TcoarsenR_2neighbor expects as input a mesh described by the 
...
%    Additionally, the 2-Neighbor Rule is considered.
...
jdx = elem2coarse(:,1:3);
lin = minEdgeVal(element3edges(jdx(:),:));
idx = find(~((lin(:,1)==lin(:,2)).* (lin(:,2)==lin(:,3)).* (lin(:,1)==lin(:,3))));
...
% follow 2-neighbor rule
boundinfo = edge2nodes(accumarray([element3edges(:);irregular2edges(:)],1,[length(edge2nodes) 1])==1,:);
idx = find(sum(markedEdges(adm2edges),2));
foo = 1;
while ~isempty(idx) || foo
    elem2coarse(idx,:)=[];
    %*** mark nodes to be eliminated
    markedNodes = accumarray(elements(elem2coarse(:,1:3),2),ones(size(elements(elem2coarse(:,1:3),2))),[nC 1]);
    markedNodes(boundinfo(:,2))= 2*markedNodes(boundinfo(:,2)); % count boundary edges twice 
    markedNodes(irregular(:,3)) = 2*markedNodes(irregular(:,3));% count constraints twice
    idx = find(sum(reshape(markedNodes(elements(elem2coarse(:,1:3),2)),[],3),2)<5);
    foo = 0;
end
%*** create new elements
